...
from classes.weather_data_transform import (
...
def main(current_dir: str) -> None:
    ...
    data_dir = os.path.join(
        current_dir,
        os.path.normpath(
            f"../../{settings.directories.data_dir}/{settings.directories.weather_data_dir}"
        ),
    )
    ...
    sorted_filenames = sorted(
        [
            file.split(".")[0]
            for file in os.listdir(os.path.normpath(data_dir))
            if ".parquet" in file
            and "transformed" not in file
    ...
    for file in sorted_filenames:
        ...
        save_path = os.path.join(
            os.path.normpath(data_dir), f"{file}_transformed.parquet"
        )
        # Transform data
        df = transform_individual_locations(file_path=file_path)
    ...
    sorted_transformed_file_dirs = [
        os.path.join(os.path.normpath(data_dir), file)
    ...
    df_combined = generate_all_information_df(df_dict)
    save_path = os.path.join(
        os.path.normpath(data_dir), "weather_data_combined.parquet"
    )
